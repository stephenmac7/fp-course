...
distinct' l =
  let p = state' . lift2 (lift2 (,)) S.notMember S.insert
...
distinctF l =
  let p x =
        ...
        if x > 100
          then const Empty
          else Full <$> lift2 (,) (S.notMember x) (S.insert x)
...
instance Monad f => Applicative (OptionalT f) where
  ...
  OptionalT f <*> OptionalT a = OptionalT $ onFull ((<$> a) . (<$>)) =<< f
